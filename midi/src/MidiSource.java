...
public final class MidiSource implements Runnable{
    ...
    public static boolean mappingKey = false;
    public static String fileMappingName = null;
    ...
    public void run() {
        ...
        x = m.x;
        y =m.y;
        System.out.println(" X : "+MidiSource.x);
        System.out.println(" Y : "+MidiSource.y);
    }
    ...
    //static boolean xyswitch = false;

    private static void passRecordKey(int keyNum) throws IOException {
        Window.enableRecordButton();
        StartUpManager.c.setRecordKey(keyNum);
        settingKey = false;
    }

    private static void passMappingKey(int keyNum){
        StartUpManager.c.recieveMappedKey(keyNum,fileMappingName);
        mappingKey = false;
    }


    static int x,y;
    private static void print(MidiMessage message) throws AWTException, InterruptedException, IOException {
        ...
        if (message instanceof ShortMessage) {
            ...
            if (shortMessage.getCommand() != 240) { // some MIDI controllers continousouly output a 240
                ...
                if (shortMessage.getCommand() ==  ShortMessage.NOTE_ON) {
                    ...
                    if(isRecording&&(shortMessage.getData1()==StartUpManager.c.getRecordKey())){
                        System.out.println("\n\nStopRecording");
                        isRecording = false;
                    }
                    else if(shortMessage.getData1()==StartUpManager.c.getRecordKey()){
                        System.out.println("\n\nStartRecording");
                        isRecording = true;
                    }
                    else if(settingKey){
                        passRecordKey(shortMessage.getData1());
                    }
                    else if(mappingKey){
                        passMappingKey(shortMessage.getData1());
                    }
    ...
    public static void pressKey(int keyNum) throws AWTException {

    }

    public static void releaseKey(int keyNum) throws AWTException {

    }

    static int xLocation = 1280;
    static int yLocation = 800;
    public static void padMouse(int keyNum, int velocity) throws AWTException, InterruptedException {
        Robot r = new Robot();
        if (keyNum > 35 && keyNum < 43) {
            ...
            r.mouseMove(xLocation,yLocation);
        }
    }
    ...
    private class MidiKeyboardControllerReceiver implements Receiver {
        ...
        public void send(MidiMessage message, long timeStamp) {
            ...
            if (verbose) {
                try {
                    print(message);
                } catch (AWTException | InterruptedException | FileNotFoundException e) {
                    e.printStackTrace();
                } catch (IOException e) {
                    e.printStackTrace();
                }
